...
// NewLazyTx creates a new LazyTx instance
func NewLazyTx(db *sqlx.DB) *LazyTx {
	return &LazyTx{db: db}
}

// beginTx lazily starts a transaction if it hasn't been started yet
...
// Commit commits the transaction if it was started
...
// Rollback rolls back the transaction if it was started
...
// Rebind implements the ReadDBExecutor interface and can work with either tx or db
func (lt *LazyTx) Rebind(query string) string {
	if lt.started {
		return lt.tx.Rebind(query)
	}
	return lt.db.Rebind(query)
}
